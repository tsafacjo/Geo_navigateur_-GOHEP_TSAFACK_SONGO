...
var Simbla = (function () {
  ...
  if (_simbla.Object.prototype.validate.toString().indexOf('g-recaptcha') == -1)
    _simbla.Object.prototype.validate = function(e){if(e.hasOwnProperty("ACL")&&!(e.ACL instanceof A["default"]))return new I["default"](I["default"].OTHER_CAUSE,"ACL must be a Parse ACL.");for(var t in e)if(!/^[A-Za-z][0-9A-Za-z_]*$/.test(t)&&t!="g-recaptcha-response")return new I["default"](I["default"].INVALID_KEY_NAME);return!1};
  var _appId;
  ...
  _simbla.Query.prototype.aggregate = function (groupby, group){
    ...
    return Simbla._request('GET', 'classes-aggregate/' + this.className, {where: this._where, group: group, groupby :groupby, limit: limit });
  }
...
function simblaRegister(form) {
  ...
  user.set("g-recaptcha-response", form["g-recaptcha-response"].value, {error: function (err, a){
    console.log(err, a);
  }});
...
function createSimblaObj(form) {
  ...
  serialize.forEach(function (e) {
    ...
    switch (elem.type) {
      case 'select-one':
       if (value && $(elem).hasClass('select-pointer')){
    ...
    addCriteriaToQuery(query, cls, c).aggregate(groupby, group).then(function (results){
      ...
      function getCategory(cat){
       if (typeof cat != "object")
        return decodeEntities(cat || "");


        var spliter = c.data('chart-category').split('/')
        return spliter.map(function (s){
          return cat[s]
        }).join('/');
      }
      ...
      success: function (num) {
        ...
        if (num > maxVisible) {
          ...
          pagingElem.show().empty().bootpag({
            total: Math.ceil(num / maxVisible), maxVisible: maxVisibleNum, leaps: true
...
function simblaTable(m) {
  ...
  function buildRow(d, i) {
    ...
    columns.each(function () {
      ...
      var dataName = $(this).data('field'), val;
      if (dataName == "action"){
        td.addClass('action');
        if (allowEdit)
  ...
  function query() {
    ...
    addCriteriaToQuery(query, cls, m).find(function (results, err) {
    ...
    addCriteriaToQuery((new Simbla.Query(obj)), cls, m).count({
      success: function (c) {
        ...
        if (c > getLimit()) {
          ...
          pagingElem.show().empty().bootpag({
            total: Math.ceil(num / getLimit()), maxVisible: maxVisible, leaps: true
          }).on("page", function (event, /* page number here */ num) {
            page = num;
            query();
          });
  ...
  m.on('click', 'th', function () {
    ...
    m.find('.page-selection').empty().bootpag({
      total: Math.ceil(num / getLimit()), maxVisible: 6, leaps: true
    });
    page = 1;
    query();
  });
...
function loadGalleryData() {
  ...
  if (!cls) return;
  var obj = Simbla.Object.extend(cls);
  var image = g.data('image-field'), url = g.data('url-field'), title = g.data('title-field');
  var page = 1;
  function getSort() {
      ...
      success: function (c) {
        ...
        if (c > limit) {
          ...
          g.find('.page-selection').show().empty().bootpag({
            total: Math.ceil(c / limit), maxVisible: 6, leaps: true
          }).on("page", function (event, /* page number here */ num) {
            page = num;
            query();
          });
...
function createHoverEffectsCssRols(item, color) {
  var style = $('#hoverEffects' + item.attr('id')), sheet;
  if (style.length == 0) {
...
$(function () {
  ...
  $('.simbla-table').each(function () {
    simblaTable($(this))
  });
  $('.simbla-chart').each(function () {
    simblaChart($(this))
  });
...
$('select.select-pointer').each(function(){
  var T = $(this);
  var target = T.attr('targetclass')
  if (target){
      new Simbla.Query(target).select('Name','name', 'Key', 'key').find().then(function (d){
              T.empty();
              T.append($('<option>'));
              d.forEach(function (o){

                  T.append($('<option>').attr('value', o.id).text(o.get('Name') || o.get('name') || o.get('Key') || o.get('key')))
                  });
...
function addTooltip() {
  ...
  $('h1, h2, h3, h4, h5, h6, label, span, a, button, input p').each(function () {
    if ($(this).parents('h1, h2, h3, h4, h5, h6, label, span, a, button, input, p').length == 0)
    $(this).tooltip({
      placement: "auto top", title: function () {
